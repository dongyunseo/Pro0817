...
public class ProDao {
	...
	public ArrayList<ProDto> selecttop() {
		...
		return dtos;
	}//select * from '테이블이름' where dressname like '%검색할 이름%';
	...
	public void dressupdate(ProDto prodto) {
		...
		try {
			...
			System.out.println("수정할 상품 이름 : " + prodto.getDressname());
			pstmt.setInt(2, prodto.getPrice());
			System.out.println("수정할 상품 가격 : " + prodto.getPrice());
			pstmt.setInt(3, prodto.getDressid());
			System.out.println("수정할 상품 아이디 : " + prodto.getDressid());
			pstmt.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
